module track
    ...
    function myKernelRegression(k::Kernel, X::Vector{Trejactory}, y::Vector{Float64}, Xstar::Vector{Trejactory}, s2::Float64)
        K = kernelmatrix(k, X)
        kstar = kernelmatrix(k, Xstar, X)
        return kstar * ((K + s2 .* I) \ y)
    end
    function PP1(x::Vector{Float64}, X::Vector{Trejactory}, y::Vector{Float64}, kernel::Kernel)
      p = x[1]
      s2 = x[2]
      k = p * kernel
